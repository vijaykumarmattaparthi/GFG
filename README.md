...
**Pipeline:** `User query → FastAPI → Claude claude-opus-4-5 (spec generation) → Query Engine (data aggregation) → JSON → Frontend (Chart.js render)`
...
- **AI**: Anthropic Claude claude-opus-4-5 (query interpretation + chart spec generation)
...
|-----------|-------------------|
| **Accuracy (40pts)** | Claude generates structured chart specs; a deterministic query engine executes them against real data — no hallucinated numbers |
